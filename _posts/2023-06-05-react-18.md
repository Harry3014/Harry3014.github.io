...
不再使用 ReactDOM.render，而是先创建 ReactDOMRoot。
...
React 18 之前，我们仅把 React event handler 的更新做批处理。Promise、setTimeout、native event handler，或者其他事件中的更新，React 默认不进行批处理：
...
- 同步类型，优先级最高，类似 click 这样的事件等等

```js
SyncLane = 0b0000000000000000000000000000001;
```

- 连续输入，类似 scroll 这样的事件
...
// ...
TransitionLane16 = 0b0000000010000000000000000000000;
```

- retry，Suspense 等待的内容解决后重新渲染时会使用

```js
RetryLane1 = 0b0000000100000000000000000000000;
// ...
RetryLane4 = 0b0000100000000000000000000000000;
```
...
function performConcurrentWorkOnRoot(root, didTimeout) {
  ...
  shouldTimeSlice = !includesBlockingLane(root, lanes) && !includesExpiredLane(root, lanes) && !didTimeout;
...
第一次遍历，测试条件是判断 key 是否匹配，如果匹配，假设 type 也匹配，那么考虑重用，否则重新创建 fiber（同时删除旧的 fiber），key 不匹配的话就跳出第一次遍历。
...
useEffect 是在 DOM 变更后执行，但是不一定阻塞浏览器渲染，如果是类似 click 这样高优先级更新，那么可能在绘制之前执行。
...
执行 effect 的过程从 fiber 树的底层开始，因为子节点比父节点先完成渲染。
...
nonDelegatedEvents = new Set(["cancel", "close", "invalid", "load", "scroll", "toggle"].concat(mediaEventTypes));
```
...
他是一个高阶组件，默认是属性的浅比较，返回的组件实际是一个对象，`$$typeof`属性是`REACT_MEMO_TYPE`，在 beginWork 处理时，会比较新旧属性。

如果你的组件在传入相同的属性会渲染相同的内容时才能使用。
...
在这个范例中，memo 实际没有起作用，因为每次渲染 Parent 时都创建了不同的 onClick 函数，使用浅比较时不会通过（Object.is）。此时应该考虑使用 useCallback。

## forwardRef

forwardRef(render)，返回值其实也是一个对象，一样有`$$typeof`属性。在 beginWork 阶段调用`render(prop, ref)`，ref 是父组件传递来的。

## createPortal

`createPortal(children, domNode, key?)`

注意这是 react-dom 的 API，返回的也是对象，一样有`$$typeof`属性。

在提交阶段处理 portal fiber 的 children 时，会将 children 对应的 dom 添加到 portal fiber 对应的 dom 上，也就是传入的 domNode 参数。实际上 domNode 参数是绑定到了 portal fiber 的 `fiber.stateNode.containerInfo` 属性上，这与 HostRoot 是一样的。

## useCallback 与 useMemo

共同点：两者都是为了记忆化某些内容。在没有传入依赖时，无法记忆相应的内容。

不同点：useMemo 会调用提供的函数得到需要记忆的内容，useCallback 不调用。

useMemo 传递的 calculator 函数在渲染阶段调用，所以需要是纯函数，而且需要返回值，不然就没意义了。

## 严格模式

仅仅在开发环境下起作用，可以只为部分开启严格模式。

- 在使用了历史遗留（过时的） API 时发出警告

- 渲染两次，如果是纯函数，那么得到的结果应该是相同的

- 在提交阶段结束后，执行副作用时，会尝试运行 effect 的 cleanup 函数，然后再次运行一次 setup，如果没有提供 cleanup 函数，那么 setup 会执行两次

## class 组件生命周期

`constructor(props)`，只在挂载时调用

下面的生命周期在运行前，updateQueue 已经处理完成（已经得到了新的 state）。

`static getDerivedStateFromProps(newProps, currentState)`返回一个对象更新当前的 state，在挂载和更新时都会调用

`shouldComponentUpdate(newProps, newState)`只在更新时调用，使用`this.forceUpdte(callback?)`时也不会调用，返回 false 后接下来的生命周期都不再执行，而且会尝试从 beginWork 提前退出。

`render`是唯一必须实现的方法。

以上都是在渲染阶段的 beginWork 调用。

`getSnapshotBeforeUpdate(currentProps, currentState)`发生在修改 DOM 之前，返回的内容会作为`componentDidUpdate`的参数。

`componentDidUpdate(currentProps, currentState, snapshot)`发生在修改 DOM 后，但是此时浏览器还并未进行绘制。

`componentWillUmmount()`只在卸载时调用，发生在提交阶段的开始，因为 fiber 的删除在这里。

## useImperativeHandle

`useImperativeHandle(ref, create, deps?)`

这个 hook 一般与 forwardRef 配合使用。使用场景可能是：不想直接暴露 DOM 给父组件，但是需要暴露一些方法，例如 focus 这样的方法。还有一点需要注意，此时父组件传来的 ref 已经不能直接绑定在 DOM 上了，应该额外创建一个。
...
它的实现实际是一个 LayoutEffect，在挂载和更新依赖时调用 create 函数，将返回的对象赋值给`ref.current`。

## jsx 转换

在 react 内部有一个 jsx-runtime
...
- 创建 lane，创建 upate 对象，保存 update 对象到 fiber 的 updateQueue 或者 hook 的 queue，这一类 queue 都是循环链表结构，不同 update 对象之间以 next 相连

- 更新自己 fiber 的 lanes，更新所有祖先 fiber 的 childLanes，让 FiberRootNode 记住要运行的轨道，`root.pendingLanes |= lane`，仅仅先记住，下一次更新不一定是运行这条轨道

- 安排一次更新，这里要决定要运行哪些轨道，从`root.pendingLanes`中决定，从高到低取，从 Transition 这种等级开始可以批量一起，否则都是拿最高等级的 lane，这里产出`nextLanes`

- 根据`nextLanes`的优先级决定渲染任务回调函数，以及怎样安排执行这个回调函数

  - 优先级很高`SyncLane`：这次渲染是同步渲染，如果支持微任务，那在微任务中执行，不支持微任务，安排成优先级最高的宏任务，但是回调函数都是`performSyncWorkOnRoot`

  - 优先级不高：安排成不同优先级的宏任务，优先级有：`ImmediatePriority | UserBlockingPriority | NormalPriority | IdlePriority`，回调函数是`performConcurrentWorkOnRoot`

- 宏任务由 react 自己的 scheduler 调度，本质是一个最小堆。

- 在 react 的调度下进入回调

  - 决定要运行哪些轨道，一样是获取`nextLanes`

  - `performConcurrentWorkOnRoot(root, didTimeout)`，这个回调本质也不一定会时间切片，，能否切片的判断条件是`shouldTimeSlice = !includesBlockingLane(root, lanes) && !includesExpiredLane(root, lanes) && ( !didTimeout)`，因为这是作为宏任务调度的，可能会超时的，为了避免超时的任务一直被其他任务插队造成饥饿问题，所以超时的任务肯定不能切片。目前也只有 Transition 和 Suspense 能时间切片。

  - `performSyncWorkOnRoot(root)`

- 无论是哪种回调，都分两个阶段，渲染阶段和提交阶段，渲染阶段会有一个返回值`exitStatus`，如果一切正常，那么会进入提交阶段，但是也可能出现错误或者中止等等情况

- 渲染阶段的产物：根据这次渲染需要显示哪些内容，产生一棵新的 fiber tree，这棵树上包含了变化信息，比较的过程是经过优化的

- 提交节点的工作：根据渲染阶段得到的变化信息，真正修改到界面上

- `workInProgess`这个 fiber，如果`workInProgressRoot !== root || workInProgressRootRenderLanes !== lanes`，如果 root 变化或者 lanes 变化，那么要抛弃目前的 workInProgess，准备一个新的 workInProgess，会调用`createWorkInProgress`创建，这里 react 使用了多缓冲技术，可以借用`fiber.alternate`

- 进入工作循环

  - `workLoopSync`

  - `workLoopConcurrent`，会处理完一个 fiber 后会检查是否应该让出主线程，调用的是 scheduler 的方法，默认以 5ms 为一帧，超过的话需要让出主线程

  - 这两种循环都处于 try catch 之下，出现错误会进行相应的处理
